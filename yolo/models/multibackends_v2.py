...
from yolo.utils.dataloaders import exif_transpose, letterbox
from yolo.utils.general import (LOGGER, ROOT, check_requirements,
...
from yolo.utils.torch_utils import copy_attr, smart_inference_mode, time_sync
from yolo.models.experimental import attempt_download, attempt_load
...
class ONNXmodel:
    def __init__(self, model_path, device=None) -> None:
        LOGGER.info(f'Loading {model_path} for ONNX Runtime inference...')
    ...
    def forward(self, im):
        im = im.cpu().numpy() if isinstance(im, torch.Tensor) else im
...
class DetectMultiBackendv2(nn.Module):
    def __init__(self, weights='yolov5s.pt', device='cpu', dnn=False, data=None, fp16=False, fuse=True) -> None:
        super().__init__()
        w = str(weights[0] if isinstance(weights, list) else weights)
        stride = 32  # default stride
        device = torch.device(device)

        if w.endswith('pt'):  # PyTorch
            ...
            model = suffix_map[Path(w).suffix[1:]](w, device)

        self.stride = stride
        self.model = model
        self.pt = w.endswith('pt')
        self.names = names
...
class DetectMultiBackend(nn.Module):
        ...
        if pt:  # PyTorch
            ...
            for index in range(model.num_bindings):
                ...
                if model.binding_is_input(index):
                    if -1 in tuple(model.get_binding_shape(index)):  # dynamic
                        dynamic = True
                        context.set_binding_shape(index, tuple(
                            model.get_profile_shape(0, index)[2]))
                    if dtype == np.float16:
                        fp16 = True
            ...
            if saved_model:  # SavedModel
                ...
                frozen_func = wrap_frozen_graph(
                    gd, inputs="x:0", outputs="Identity:0")
                ...
                raise NotImplementedError(
                    f'ERROR: {w} is not a supported format')

        # class names
        if 'names' not in locals():
            names = yaml_load(data)['names'] if data else [
                f'class{i}' for i in range(999)]
    ...
    def forward(self, im, augment=False, visualize=False, val=False):
        ...
        if self.pt:  # PyTorch
            ...
            if self.dynamic and im.shape != self.bindings['images'].shape:
                i_in, i_out = (self.model.get_binding_index(x)
                               for x in ('images', 'output'))
                self.context.set_binding_shape(
                    i_in, im.shape)  # reshape if dynamic
            ...
            if 'confidence' in y:
                ...
                k = 'var_' + str(sorted(int(k.replace('var_', ''))
                                 for k in y)[-1])  # output key
                y = y[k]  # output
        else:  # TensorFlow (SavedModel, GraphDef, Lite, Edge TPU)
            # torch BCHW to numpy BHWC shape(1,320,192,3)
            im = im.permute(0, 2, 3, 1).cpu().numpy()
            if self.saved_model:  # SavedModel
                y = (self.model(im, training=False)
                     if self.keras else self.model(im)).numpy()
    ...
    def _model_type(p='path/to/model.pt'):
        ...
        # from yolo.tools.export import export_formats
...
class AutoShape(nn.Module):
    ...
    def forward(self, imgs, size=640, augment=False, profile=False):
        ...
        # Pre-process
        n, imgs = (len(imgs), list(imgs)) if isinstance(
        ...
        for i, im in enumerate(imgs):
            ...
            imgs[i] = im if im.data.contiguous else np.ascontiguousarray(
                im)  # update
        shape1 = [make_divisible(x, self.stride) if self.pt else size for x in np.array(
            shape1).max(0)]  # inf shape
...
class Detections:
    ...
    def save(self, labels=True, save_dir='runs/detect/exp'):
        ...
        self.display(save=True, labels=labels,
                     save_dir=save_dir)  # save results
    ...
    def tolist(self):
        ...
        x = [Detections([self.imgs[i]], [self.pred[i]], [
